-- KeywordPeek Complete Schema
-- Combined initial schema + credit expiry updates
...
-- Credit Transactions (with expiry support)
create table public.transactions (
  ...
  expires_at timestamptz default null,  -- When credits expire. NULL = never expires
  created_at timestamptz default now() not null
);
...
comment on column public.transactions.expires_at is 'When credits expire. NULL = never expires (for grandfathered/bonus credits)';
...
create index idx_transactions_expires_at on public.transactions(expires_at) where expires_at is not null;
...
-- Add credits with expiry support
...
comment on function public.add_credits is 'Add credits to user with optional 1-year expiry for purchases';
...
-- Get credits expiring within N days (for reminder emails)
...
comment on function public.get_expiring_credits is 'Get credits expiring within specified days for reminder emails';
...
comment on function public.handle_new_user is 'Create profile with 10 free credits on user signup';
